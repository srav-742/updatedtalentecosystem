...
const ApplicationFlow = () => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            if (method === 'upload') {
                ...
                const extractRes = await axios.post('http://127.0.0.1:5000/api/extract-pdf', formData);
            ...
            // Analyze with Gemini
            const res = await axios.post('http://127.0.0.1:5000/api/analyze-resume', {
                resumeText,
            ...
            // Save application
            await axios.post('http://127.0.0.1:5000/api/applications', {
                jobId,
    ...
    const startAssessment = async () => {
        ...
        try {
            ...
            const res = await axios.post('http://127.0.0.1:5000/api/generate-full-assessment', {
                jobTitle: job.title,
                jobSkills: job.skills,
            ...
            const fallbackQs = Array.from({ length: job.assessment?.totalQuestions || 5 }, (_, i) => ({
                title: `${fallbackSkills[i % fallbackSkills.length]} Advanced Assessment`,
                question: `In a production ${fallbackSkills[i % fallbackSkills.length]} environment, which protocol ensures maximum reliability and scalability for this module?`,
    ...
    const startInterview = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.post('http://127.0.0.1:5000/api/generate-interview-questions', {
    ...
    const submitAssessment = async () => {
        ...
        try {
            ...
            if (job?.mockInterview?.enabled) {
                ...
                await axios.post('http://127.0.0.1:5000/api/applications', applicationData);
                setStep(6);
            }
        } catch (error) {
    ...
    const handleInterviewSubmit = async () => {
        ...
        try {
            stopCamera();
            const res = await axios.post('http://127.0.0.1:5000/api/analyze-interview', {
